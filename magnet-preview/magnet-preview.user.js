...
function WL(m){return GM('https://whatslink.info/api/v1/link?url='+encodeURIComponent(m)).then(d=>{if(d.error)throw Error(d.error);return{name:d.name,size:d.size,count:d.count,type:d.file_type,shots:(d.screenshots||[]).map(s=>s.screenshot)};});}

function IT(h){return GM('https://itorrents.org/torrent/'+h.toUpperCase()+'.torrent',{t:10000,b:true}).then(raw=>{const info=PT(new Uint8Array(raw));if(!info)throw Error('parse');return{name:info.name,files:info.files,total:info.total};});}
...
function BD(h){return GM('https://btdig.com/'+h,{t:12000}).then(html=>{
...
const h=H(magnet);
const src=[],sent=new Set();
function done(p){
if(!sent.has(p)){sent.add(p);spin.style.display='none';cp.style.display='';}
if(p==='info'){infoEl.innerHTML='';if(R.size)infoEl.innerHTML+='<span>📀 '+F(R.size)+'</span>';if(R.type&&R.type!=='unknown')infoEl.innerHTML+='<span class="mp-tag">'+E(R.type)+'</span>';if(R.count)infoEl.innerHTML+='<span>📁 '+R.count+' 个文件</span>';if(src.length)infoEl.innerHTML+='<span class="mp-src">🔗 '+src.join(' + ')+'</span>';title.textContent=R.name||'磁力链接预览';}
if(p==='files'){if(R.files.length){filesWrap.innerHTML='<div class="mp-sec">📂 文件列表</div><div class="mp-files">'+R.files.map(f=>'<div class="mp-file"><span>'+I(f.name)+'</span><span title="'+E(f.name)+'">'+E(f.name)+'</span><span>'+F(f.size)+'</span></div>').join('')+'</div>';}}
if(p==='shots'){if(R.shots.length){shotsWrap.innerHTML='<div class="mp-sec">🖼 预览截图 ('+R.shots.length+'张)</div><div class="mp-shots">'+R.shots.map(s=>'<img class="mp-shot" data-src="'+E(s)+'" src="" onerror="this.style.display=\'none\'">').join('')+'</div>';lazyLoad(shotsWrap);}}
}
const R={name:'',size:0,count:0,type:'',shots:[],files:[]};
WL(magnet).then(d=>{Object.assign(R,{name:d.name,size:d.size,count:d.count,type:d.type,shots:d.shots});src.push('whatslink');done('info');if(d.shots?.length)done('shots');}).catch(()=>{});
if(h){IT(h).then(d=>{R.name=R.name||d.name;R.files=d.files;R.count=R.files.length||R.count;if(!R.size)R.size=d.total;src.push('itorrents');done('files');}).catch(()=>{});
BD(h).then(d=>{if(!R.files.length&&d.files){R.files=d.files;src.push('btdig');done('files');}}).catch(()=>{});}
setTimeout(()=>{if(!sent.size){spin.style.display='none';body.innerHTML='<div class="mp-empty">未能获取种子信息</div>';cp.style.display='';}},5000);
}
...
.mp-btn-copy{color:#2563eb}.mp-btn-copy:hover{background:#eff6ff;color:#1d4ed8}
.mp-body{padding:20px;overflow-y:auto;flex:1}
.mp-spin{width:32px;height:32px;margin:40px auto;border:3px solid #f1f5f9;border-top-color:#7c3aed;border-radius:50%;animation:mp-spin .8s linear infinite}
.mp-empty{text-align:center;padding:40px 20px;color:#dc2626}
.mp-info{display:flex;gap:12px;padding:12px 16px;background:#f8fafc;border-radius:10px;margin-bottom:16px;border:1px solid #f1f5f9;font-size:12px;color:#64748b;flex-wrap:wrap;align-items:center;min-height:40px}
...
.mp-shots-wrap,.mp-files-wrap{min-height:0}
`.trim());
})();
