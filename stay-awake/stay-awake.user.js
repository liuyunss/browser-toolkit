...
// @version      1.0.0
// @description  防止网页因闲置超时退出登录，支持指定网站生效
// @author       liuyunss
// @match        *://*/*
// @grant        GM_getValue
// @grant        GM_setValue
...
(function () {
  ...
  function shouldRun() {
    ...
    if (list.length === 0) return true; // 未配置则所有站生效
  ...
  function simulateKey() {
    const keys = [16, 17, 18]; // Shift, Ctrl, Alt
  ...
  function openSettings() {
    ...
    <div class="m"><div class="p">
      ...
      <div class="r" style="flex-direction:column;align-items:stretch"><div style="margin-bottom:6px"><label>生效网站</label><div class="h">逗号分隔，留空则所有站生效。支持通配符如 *.example.com</div></div><textarea class="tx" id="s-sites" placeholder="example.com, .company.com">${c.sites}</textarea></div>
    ...
    sh.getElementById('s-ok').addEventListener('click', e => {
      ...
      saveCfg({
        ...
        sites:    sh.getElementById('s-sites').value.trim(),
      });
      closeSettings();
